...
export function generateTokens(payload) {
    const accessToken = jwt.sign({ ...payload, type: 'access' }, JWT_SECRET, { expiresIn: process.env.JWT_EXPIRES_IN || '24h' });
    const refreshToken = jwt.sign({ ...payload, type: 'refresh' }, JWT_REFRESH_SECRET, { expiresIn: process.env.JWT_REFRESH_EXPIRES_IN || '7d' });
    return {
        accessToken,
        refreshToken,
 ...
 */
export async function extractCampusContext(req, res, next) {
    try {
        ...
        if (subdomain && subdomain !== 'www' && subdomain !== 'api') {
            const campus = await cacheService.get(`campus:subdomain:${subdomain}`);
            if (campus) {
                campusId = campus.id;
            }
            else {
