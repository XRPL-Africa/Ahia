...
import { UserStatus, UserRole, EscrowStatus } from '@prisma/client';
export class AdminService {
    ...
    async getDashboardStats() {
        ...
        const [totalUsers, verifiedUsers, pendingUsers, bannedUsers, newUsersToday, totalListings, activeListings, soldListings, newListingsToday, totalEscrows, activeEscrows, completedEscrows, disputedEscrows, escrowVolume, totalTransactions, transactionVolume, platformFees,] = await Promise.all([
            prisma.user.count(),
     ...
     * Add strike to user
     */
    async addStrike(userId, reason, adminId) {
        const user = await prisma.user.findUnique({
            where: { id: userId },
        });
        if (!user) {
     ...
     * Resolve dispute
     */
    async resolveDispute(disputeId, resolution, notes, adminId) {
     ...
     * Update campus
     */
    async updateCampus(campusId, input) {
