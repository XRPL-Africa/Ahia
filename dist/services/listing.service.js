...
export class ListingService {
    LISTING_CACHE_TTL = 300; // 5 minutes
    LISTING_EXPIRY_DAYS = 30;
    /**
     * Create a new listing
     */
    async createListing(sellerId, input, images) {
     ...
     * Get listing by ID
     */
    async getListingById(listingId, incrementViews = false) {
     ...
     */
    async getListings(filters, sort = { field: 'createdAt', order: 'desc' }, page = 1, limit = 20) {
     ...
     * Update listing
     */
    async updateListing(listingId, sellerId, data) {
    ...
    async createBid(buyerId, input) {
        ...
        // Notify seller
        await notificationService.notifyNewBid(listing.sellerId, listing.title, input.amount);
        // Clear cache
     ...
     */
    async respondToBid(sellerId, bidId, action, counterAmount, counterMessage) {
     ...
     * Get user's listings
     */
    async getUserListings(userId, status, page = 1, limit = 20) {
        const skip = (page - 1) * limit;
        const where = {
            sellerId: userId,
            ...(status && { status }),
     ...
     * Get user's bids
     */
    async getUserBids(userId, status, page = 1, limit = 20) {
        const skip = (page - 1) * limit;
        const where = {
            buyerId: userId,
            ...(status && { status }),
