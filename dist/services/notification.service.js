...
export class NotificationService {
     ...
     */
    async createBulkNotifications(userIds, input) {
        try {
    ...
    async markAsRead(notificationId, userId) {
        ...
        if (!notification.isRead) {
            ...
            const cacheKey = `notifications:unread:${userId}`;
            const currentCount = await cacheService.get(cacheKey);
     ...
     */
    async notifyVerification(userId, status, reason) {
     ...
     */
    async notifyNewBid(sellerId, listingTitle, bidAmount) {
        await this.createNotification({
            userId: sellerId,
            type: 'BID',
     ...
     */
    async notifyBidResponse(buyerId, listingTitle, action) {
        const messages = {
     ...
     */
    async notifyEscrowStatus(userId, status, listingTitle) {
        const messages = {
     ...
     */
    async notifyPaymentReceived(userId, amount, listingTitle) {
        await this.createNotification({
            userId,
            type: 'PAYMENT',
     ...
     */
    async notifyStrike(userId, strikeCount, reason) {
        await this.createNotification({
            userId,
            type: 'SYSTEM',
     ...
     */
    async notifySuspension(userId, reason, duration) {
        await this.createNotification({
            userId,
            type: 'SYSTEM',
