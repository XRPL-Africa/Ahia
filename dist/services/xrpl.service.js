...
class XRPLService {
    ...
    constructor() {
        ...
        if (process.env.XRPL_PLATFORM_SEED) {
            this.platformWallet = xrpl.Wallet.fromSeed(process.env.XRPL_PLATFORM_SEED);
        }
        else {
        ...
        if (process.env.XRPL_ISSUER_SEED) {
            this.issuerWallet = xrpl.Wallet.fromSeed(process.env.XRPL_ISSUER_SEED);
        }
        else {
    ...
    // 🔥 GENERIC TX HANDLER
    async submitTransaction(wallet, tx) {
    ...
    // 🔥 CREATE TRUSTLINE
    async createTrustline(walletSeed) {
    ...
    async issueRLUSD(destination, amount) {
    ...
    async transferRLUSD(destination, amount) {
        if (!this.platformWallet)
            throw new Error("Platform wallet not initialized");
        const tx = {
            TransactionType: "Payment",
    ...
    async releaseFunds(destination, amount) {
        return this.sendXRP(destination, amount);
    }
    async refundFunds(destination, amount) {
        return this.sendXRP(destination, amount);
    }
}
