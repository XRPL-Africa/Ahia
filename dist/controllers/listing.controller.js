...
import { createListingSchema, updateListingSchema, listingFiltersSchema, createBidSchema, bidResponseSchema, uuidParamSchema, } from '../validators/index.js';
...
export const listingController = {
    ...
    getListings: asyncHandler(async (req, res) => {
        ...
        const result = await listingService.getListings(filters, { field: sortBy, order: sortOrder }, page, limit);
        res.json({
            success: true,
     ...
     */
    respondToBid: asyncHandler(async (req, res) => {
        const { id: bidId } = uuidParamSchema.parse(req.params);
        const { action } = bidResponseSchema.parse(req.body);
        const bid = await listingService.respondToBid(req.user.id, bidId, action, req.body.counterAmount, req.body.counterMessage);
        res.json({
            success: true,
    ...
    getMyListings: asyncHandler(async (req, res) => {
        ...
        const result = await listingService.getUserListings(req.user.id, status, page, limit);
        res.json({
            success: true,
     ...
     * Get user's bids
     */
    getMyBids: asyncHandler(async (req, res) => {
        const page = parseInt(req.query.page) || 1;
        const limit = parseInt(req.query.limit) || 20;
        const status = req.query.status;
        const result = await listingService.getUserBids(req.user.id, status, page, limit);
        res.json({
            success: true,
     ...
     * Get campus listings
     */
    getCampusListings: asyncHandler(async (req, res) => {
        const { campusId } = req.params;
        const validatedQuery = listingFiltersSchema.parse(req.query);
        const { page = 1, limit = 20, sortBy = 'createdAt', sortOrder = 'desc', ...filters } = validatedQuery;
        const result = await listingService.getListings({ ...filters, campusId }, { field: sortBy, order: sortOrder }, page, limit);
        res.json({
            success: true,
