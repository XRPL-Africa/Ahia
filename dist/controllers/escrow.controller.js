...
import { createEscrowSchema, freezeEscrowSchema, createDisputeSchema, uuidParamSchema, } from '../validators/index.js';
export const escrowController = {
    ...
    getMyEscrows: asyncHandler(async (req, res) => {
        ...
        const escrows = await escrowService.getUserEscrows(req.user.id, role || 'all', status);
        res.json({
            success: true,
     ...
     */
    freezeEscrow: asyncHandler(async (req, res) => {
        const { id } = uuidParamSchema.parse(req.params);
     ...
     * Open dispute
     */
    openDispute: asyncHandler(async (req, res) => {
        const validatedData = createDisputeSchema.parse(req.body);
        const escrow = await escrowService.openDispute(validatedData.escrowId, req.user.id, validatedData.reason, validatedData.description, req.body.evidenceUrls);
        res.status(201).json({
            success: true,
     ...
     */
    handlePaystackWebhook: asyncHandler(async (req, res) => {
