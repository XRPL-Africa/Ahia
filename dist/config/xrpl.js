...
class XRPLService {
     ...
     * TRUSTLINE (SAFE)
     */
    async setTrustLine(wallet, limit = "100000") {
    ...
    async transferRLUSD(senderSeed, destination, amount) {
        const client = await this.ensureClient();
    ...
    async getRLUSDBalance(address) {
        ...
        const line = response.result.lines.find((l) => l.currency === RLUSD_CURRENCY &&
